...
export const HeroSection = () => {
  ...
  return <section className="relative min-h-screen flex flex-col items-center justify-center overflow-hidden bg-background px-4">
      {/* Decorative floating elements */}
      <div className="absolute inset-0 pointer-events-none overflow-hidden">
        <span className="absolute top-[10%] left-[5%] text-4xl md:text-5xl animate-float opacity-20">♪</span>
        <span className="absolute top-[15%] right-[10%] text-5xl md:text-6xl animate-float opacity-15" style={{
        animationDelay: "1s"
      }}>♫</span>
        <span className="absolute bottom-[25%] left-[8%] text-3xl md:text-4xl animate-float opacity-20" style={{
        animationDelay: "2s"
      }}>♬</span>
        <span className="absolute top-[35%] left-[20%] text-4xl animate-float opacity-10" style={{
        animationDelay: "0.5s"
      }}>🎵</span>
        <span className="absolute bottom-[35%] right-[15%] text-5xl animate-float opacity-15" style={{
        animationDelay: "1.5s"
      }}>🎶</span>
      </div>
      ...
      <div className="w-full max-w-6xl mx-auto flex-1 flex items-center justify-center py-8 md:py-16">
        <div className={cn("relative w-full transition-all duration-1000", isLoaded ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10")}>
          {/* Hero image */}
          <img alt="Easton Romero - Game Music Producer" className="w-full h-auto max-h-[75vh] object-contain mx-auto rounded-none shadow-none opacity-100" src="/uploads/1245b56e-602c-4e69-a193-52d86b577d96.png" />
      ...
      {/* Scroll indicator */}
      <div onClick={scrollToMusic} className={cn("pb-8 cursor-pointer transition-all duration-700 delay-500", "hover:scale-110 active:scale-95", isLoaded ? "opacity-100 translate-y-0" : "opacity-0 translate-y-5")}>
